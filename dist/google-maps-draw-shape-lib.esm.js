import { simplify, geom, operation } from 'jsts/dist/jsts';
  ...
  if ((shape == null ? void 0 : shape.length) > 2) {
    ...
    for (var i = 0; i < shape.length; i++) {
      if (!firstCoordinate) {
        ...
        polygon = [];
        polygon.push(validate ? new geom.Coordinate(shape[i].lng, shape[i].lat) : new google.maps.LatLng(shape[i].lat, shape[i].lng));
        continue;
      }
...
Object.defineProperty(JstsHelper, _simplifyPolygon, {
  ...
  value: function value(polygon, zoom) {
    ...
    return simplify.TopologyPreservingSimplifier.simplify(polygon, tolerance);
  }
});
Object.defineProperty(JstsHelper, _convertFromJstsGeometry, {
  writable: true,
  value: function value(geom$1) {
    var polygons = []; // Sets shape on clockwise order

    geom$1.normalize();

    if (geom$1 instanceof geom.Polygon) {
      var polygon = _classPrivateFieldLooseBase(JstsHelper, _convertFromJstsCoordinates)[_convertFromJstsCoordinates](geom$1.getCoordinates());

      polygons.push(polygon);
    }

    if (geom$1 instanceof geom.MultiPolygon) {
      for (var n = geom$1.getNumGeometries(); n > 0; n--) {
        var _polygon = _classPrivateFieldLooseBase(JstsHelper, _convertFromJstsCoordinates)[_convertFromJstsCoordinates](geom$1.getGeometryN(n - 1).getCoordinates());
...
Object.defineProperty(JstsHelper, _validateGeometry, {
  ...
  value: function value(geom$1) {
    if (geom$1 instanceof geom.Polygon) {
      ...
      var polygonizer = new operation.polygonize.Polygonizer();

      _classPrivateFieldLooseBase(JstsHelper, _addPolygon)[_addPolygon](geom$1, polygonizer);

      return _classPrivateFieldLooseBase(JstsHelper, _toPolygonGeometry)[_toPolygonGeometry](polygonizer.getPolygons());
    } else if (geom$1 instanceof geom.MultiPolygon) {
      if (geom$1.isValid()) {
        return geom$1;
      }

      var _polygonizer = new operation.polygonize.Polygonizer();

      for (var n = geom$1.getNumGeometries(); n > 0; n--) {
        _classPrivateFieldLooseBase(JstsHelper, _addPolygon)[_addPolygon](geom$1.getGeometryN(n - 1), _polygonizer);
      }
 ...
 */
function MapDrawShapeManager(_map, _callback, drawingMode, drawFreeHandMode, polygonOptions, initialPointInnerHtml, deletePointInnerHtml) {
  var _this = this;
  ...
  Object.defineProperty(this, _getZoomByBounds, {
    writable: true,
    value: function value(map, bounds) {
...
//# sourceMappingURL=google-maps-draw-shape-lib.esm.js.map
